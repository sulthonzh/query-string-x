...
function parseJSON<T = any>(input: string): T {
  try {
...
function parseCLIOptions(args: string[]): { options: CLIOptions; rest: string[] } {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith('--')) {
      ...
      switch (key) {
        case 'array-format':
          options.arrayFormat = value as 'brackets' | 'repeat' | 'comma'
          break
...
function getInputFromArgs(args: string[], options: { inputFile?: string }): string {
...
function writeOutput(output: any, options: { outputFile?: string; pretty?: boolean; color?: boolean }): void {
  const jsonOptions: any = options.pretty ? { space: 2 } : {}
...
function main(): void {
  ...
  try {
    ...
    const cleanArgs = rest.filter(arg => 
